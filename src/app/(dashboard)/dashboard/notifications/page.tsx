...
export default function UserNotificationsPage() {
    ...
    async (page: number, append = false) => {
      try {
        ...
        const res = await fetch(
          `/api/user/notifications?page=${page}&limit=${PAGE_SIZE}`,
          { cache: "no-store" }
        );
        if (!res.ok) return;
        ...
        if (append) {
          setAllNotifications((prev) => [...prev, ...(json.data ?? [])]);
        } else {
  ...
  const { today, yesterday, earlier } = groupNotifications(displayedNotifications);
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
              ...
              {tab === "unread" && allNotifications.length > 0 && (
                <button
                  onClick={() => setTab("all")}
          ...
          {/* Load more — only shown in "all" tab */}
          {tab === "all" && pagination?.hasMore && (
